...
//    protected String requst_tag;//作为请求的id，防止在多个请求同时进行时数据传递错误。
    ...
    // 增加 Volley 网络请求失败回调接口，登陆选择鼎葵是咨询为空，提示用户联系供应商
    //----------------------------------------------------------------------------------------------------------
    /**
     * 处理 Volley 网络请求失败的接口
     */
    ...
    public BaseAsyncHttpClient(BaseActivityListener activityListenerw, AsyncHttpCallback callback) {

         mContext = activityListenerw.getMContext();
...
//        }
        ...
        activityListener = activityListenerw;
...
//            params.put("UUID",uuid.toString() );//给每一个接口请求添加唯一识别码参数UUID
        ...
        MLog.i(log_msg);
     /*   if (need_show_dialog){
           new BaseToastDialogFragmentActivity().showLoadingDialog();
        }*/
        try {
